...
function FillCertification() {
  ...
  const goToEdit = async () => {
    ...
    try {
      ...
      const updateUserResponse = await axios.put(`${process.env.REACT_APP_SERVER_API}/api/users/begincertificate`,{
        userId: user._id,
  ...
  useEffect(() => {
    const fetchCertificate = async () => {
      try {
        const response = await axios.get(`${process.env.REACT_APP_SERVER_API}/api/certifications/certificate/${id}`);
  ...
  const openPdfInNewTab = async (pdfUrl) => {
    try {
      const response = await axios.get(`${process.env.REACT_APP_SERVER_API}/api/document/${pdfUrl}`, { responseType: 'arraybuffer' });
